                    ...
                    if(treeNode != null){
                        ...
                        if(treeNode.checked){
                            ...
                            if($(that.getSelectedWidget()).find("li[data_value="+treeNode.id+"]").length < 1){
                                // 将选中的机构或者 个人，从右边移除
                                $(treeSelectedSelector + " ul").append("<li style='position: relative;' id='"+treeNode.tId+"_right_li' data_value='"+treeNode.id+"' parent_data_value='"+treeNode.pId+"'>"+treeNode.name+"<span style=\"position:absolute;right: 0;\">X</span></li>")
        ...
        this.showInitSelectedNodes = function(){
            ...
            if(that.setting.modalType == "both"){
                ...
                for(var i = 0; i < resultLength; i++){
                    ...
                    resultHtml = resultHtml + '<li style=\'position: relative;\' data_value="'+tempNode["id"]+'" parent_data_value="'+tempNode["pId"]+'">'+tempNode["name"]+'<span style="position:absolute;right: 0;">X</span></li>';
                }
                // 将HTML代码放到指定的位置中
