define(["JQuery"], function($){
    var init = function(option){
        ...
        var $wrap_box = $(_default.picker);

        //配置外部容器css，方便内部定位
        $wrap_box.css({
            'position':'relative',
            'display':'inline-block',
            'width':(_default.width < 190 ? 190 : _default.width)+'px',
            'height':_default.height+'px'
        });

        //在内部添加一个城市选择器和一个城市选择块
        $wrap_box.html('<input class="wp-cityPicker content" name="'+_default.formName+'" placeholder="请选择城市"/>' +
            '<div class="wp-cityBox" style="top:'+(_default.height)+'px">' +
            '<div class="level1-box"></div><div class="level2-box"></div>' +
            '</div>');

        //获取全局的jq对象
        var $inputPicker = $wrap_box.find('.wp-cityPicker');
        var $cityBox = $wrap_box.find('.wp-cityBox');
        var $level1Box = $cityBox.find('.level1-box');
        var $level2Box = $cityBox.find('.level2-box');
        var level1Cache = {};

        //加载一级省市
        var level1 = '';
        for(var i=0;i < cityData.length;i++){
            var text = cityData[i]['n'];
            level1 += '<span class="level1">'+ text +'</span>';

            if(text.indexOf('市') == -1 && text.indexOf('香港') == -1 && text.indexOf('澳门') == -1){
                level1Cache[text] = cityData[i]['s'];
            }else{
                level1Cache[text] = null;
            }
        }
        $level1Box.append(level1);

        //选择器点击展开城市块事件
        $inputPicker.off('focus').on('focus',function(e){
            $cityBox.show();
        });

        //一级城市点击事件,展开对应的二级城市
        $level1Box.find('.level1').off('click').on('click',function(e){
            var level2 = '';
            var text = $(this).text();
            var cityList = level1Cache[text];

            //判断是否本身为城市,本身之下还有子城市就跳转城市选择并加载二级城市
            if(cityList != null){
                for(var i=0;i < cityList.length;i++){
                    level2 += '<span class="level2">'+cityList[i]['n']+'</span>'
                }
                $level2Box.html(level2);

                //绑定二级城市选择事件
                $level2Box.find('.level2').off('click').on('click',function(e){
                    $inputPicker.val($(this).text());
                    $cityBox.hide();
                    $level2Box.hide();
                    $level1Box.show();
                });
        ...
        $(document).on('click',function(e){
            var $this = $(e.target);
            //如果点击区域不是城市选择区域范围就隐藏城市选择块
            //console.log(!$this.is($inputPicker),!$this.is($cityBox),!$.contains($cityBox[0],$this[0]));
            if(!$this.is($inputPicker) && !$this.is($cityBox) && !$.contains($cityBox[0],$this[0])){
                $cityBox.hide();
                $level2Box.hide();
                $level1Box.show();
            }
